...
- Revenue grew 55 times from October 2016 to March 2017 in just six months
...
```
SOURCE DATA (Kaggle CSV files)
        |
        |  Python batch_split.py
        v
MONTHLY CSV BATCHES (local folders by year-month)
        |
        |  Manual upload or AzCopy
        v
ADLS GEN2 RAW CONTAINER
        |
        |  Fabric Data Factory Pipeline
        |  [Get Metadata > Lookup > Filter > ForEach > Copy + Script]
        v
FABRIC LAKEHOUSE - LANDING (Files/landing/)
        |
        |  Notebook: landing_to_bronze.ipynb
        v
FABRIC LAKEHOUSE - BRONZE (Files/bronze/ - Delta partitioned by processing_date)
        |
        |  Notebook: bronze_to_silver.ipynb (20+ cleaning functions + DQ + MERGE)
        v
FABRIC LAKEHOUSE - SILVER (Tables/ - Delta with schema enforcement)
        |
        |  Notebook: silver_to_gold.ipynb (star schema + business metrics + MERGE)
        v
FABRIC WAREHOUSE - GOLD (dbo schema - managed Delta tables)
        |
        |  Power BI Desktop - semantic model + DAX measures
        v
POWER BI REPORT (3 pages: Executive | Sales | Operations)
```
...
| Layer | Tool | Role |
|---|---|---|
| Data Preparation | Python pandas | Monthly batch splitting using order timestamp as anchor key |
| Raw Storage | Azure Data Lake Storage Gen2 | Stores monthly CSV batch files in hierarchical namespace |
| Orchestration | Microsoft Fabric Data Factory | 7-stage pipeline from metadata retrieval to Gold transformation |
| File Audit | Fabric Warehouse file_tracker | Prevents duplicate ingestion and provides full audit trail |
| Landing Zone | Fabric Lakehouse Files/landing/ | Temporary staging deleted after Bronze write succeeds |
| Bronze Layer | Fabric Lakehouse Files/bronze/ | Raw Delta tables partitioned by processing_date |
| Transformation | PySpark in Fabric Notebooks | 20+ reusable cleaning functions, DQ checks, schema casting |
| Silver Layer | Fabric Lakehouse Tables/ | Cleaned validated Delta tables with upsert merge pattern |
| Gold Layer | Fabric Warehouse dbo schema | Star schema dim and fact tables with calculated business columns |
| Semantic Model | Power BI Desktop | Star schema relationships, 30+ DAX measures, disconnected tables |
| Reporting | Power BI Report | 3-page interactive report for executive, sales, and operations |
| Version Control | GitHub | All notebooks, scripts, and documentation |

---

## 11. GitHub Repository Guide
...
|
+-- 01_data_preparation/
|   +-- batch_split.ipynb                   <- Monthly batch splitting script
|   +-- README.md
|
+-- 02_pipeline/
|   +-- orchestration_pipeline.png          <- Pipeline screenshot from Fabric
|   +-- pipeline_activities.md              <- Activity-by-activity explanation
|   +-- file_tracker_schema.sql             <- DDL for file_tracker table
|
+-- 03_notebooks/
...
|   +-- order_shipment_performance.png
|
+-- 06_architecture/
...
```bash
git clone https://github.com/yourusername/olist-data-engineering-platform
cd olist-data-engineering-platform
```

**Step 2 — Run the batch splitting script**

Open `01_data_preparation/batch_split.ipynb`, update the file paths to your local Olist CSV files, and run all cells. This creates a `batches/` folder with monthly CSV files.

**Step 3 — Upload batches to ADLS Gen2**

```bash
azcopy copy "batches/*" "https://yourstorage.blob.core.windows.net/raw" --recursive
```

**Step 4 — Configure and run the Fabric pipeline**

Import the pipeline into Microsoft Fabric Data Factory, configure the ADLS Gen2 and Lakehouse connections, then trigger the pipeline with the `batch_month` parameter e.g. `2016-10`.

**Step 5 — Notebooks run automatically via pipeline**
...
3. Select your Fabric Warehouse
4. Load all five Gold tables
5. Apply the star schema relationships from Section 8.1
6. Create the DAX measures from `04_semantic_model/dax_measures.md`
...
# Large data files - do not commit raw CSVs
batches/
data/raw/
*.parquet
olist_*.csv
...
# Power BI binary files (too large for git)
*.pbix
```
...
|---|---|---|
| `feat:` | New feature or notebook | `feat: add silver to gold transformation notebook` |
| `fix:` | Bug or error correction | `fix: correct surrogate key generation for dim_products` |
| `docs:` | Documentation update | `docs: add data dictionary to README` |
| `refactor:` | Code restructure | `refactor: extract cleaning functions into shared module` |
| `data:` | Data or schema change | `data: add delivery_variance_days column to fact_orders` |
| `pipeline:` | Pipeline configuration change | `pipeline: add Delete activity after Bronze notebook` |
...
|---|---|
| What is total revenue and trend? | R$792K across 5,591 orders. Revenue grew 55x in 6 months. |
| Are orders delivered on time? | 89.4% on time. 592 orders arrived late. |
| Where is the delivery bottleneck? | Last mile takes 9.29 days which is 69% of total time. |
| Which categories drive revenue? | Health beauty leads at R$66K. Watches gifts has highest unit price at R$373. |
| What is customer satisfaction? | 4.02 out of 5. Improved from 3.56 in Oct 2016 to 4.07 by Mar 2017. |
| Are we over-dependent on SP? | Yes. 59.5% of all sellers are in Sao Paulo state. |
| Which files have been processed? | Full audit trail in file_tracker table with file name and load date. |
---

*Olist E-Commerce Data Engineering Platform · Solomon Yakubu · 2026*
